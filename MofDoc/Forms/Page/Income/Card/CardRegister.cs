...
{
    public partial class CardRegister : DevExpress.XtraEditors.XtraForm
    {
        ...
        public CardRegister(string dbName)
        {
            InitializeComponent();
            this.dbName = dbName;
            InitControl();
        ...
        public CardRegister(string dbName, Document document)
        {
            InitializeComponent();
            this.dbName = dbName;
        ...
        {
            this.Text = "Карттай бүртгэл(Хуучин)";
            this.MaximumSize = this.MinimumSize = this.Size;

            txtControlNum.Properties.ReadOnly = txtRegisterNum.Properties.ReadOnly = txtRegisteredBy.Properties.ReadOnly = true;
            ...
            barBtnPrintBack.ItemClick += new DevExpress.XtraBars.ItemClickEventHandler(barBtn_ItemClick);
            barBtnPrintBack.Tag = 7;
            ...
            dateReturn.Validating += new CancelEventHandler(date_Validating);
            dateReturn.Tag = 3;
            ...
            txtControlNum.Validating += new CancelEventHandler(txt_Validating);
            txtControlNum.Tag = "Карттай бичгийн дугаарыг оруулна уу.";
        ...
        private void InitData()
        {
            ...
            try
            {
                ...
                if (isEditMode)
                {
                    ...
                    lciDateType.Control.Enabled = false;
                    lciReturnDate.Control.Enabled = true;

                    dateRegistered.EditValue = document.RegDate;
                    txtRegisterNum.Text = document.RegNum;
                    txtRegisteredBy.Text = document.Name;
                    txtControlNum.Text = document.ControlNum.ToString();

                    dateDoc.EditValue = document.DocDate;
                    dateReturn.EditValue = document.ReturnDate;
                ...
                {
                    lciDateType.Control.Enabled = true;
                    lciReturnDate.Control.Enabled = false;
            ...
            {
                System.Diagnostics.Debug.WriteLine("Карттай бичиг нээхэд алдаа гарлаа: " + ex.Message);
                Tool.ShowError(ex.Message, ex.InnerException.Message);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("Карттай бичиг нээхэд алдаа гарлаа: " + ex.Message);
                Tool.ShowError("Карттай бичиг нээхэд алдаа гарлаа!", ex.Message);
            }
            finally { filter = null; info = null; filterStored = null; tableNames = null; ds = null; datatable = null; Tool.CloseWaiting(); }
        }
        ...
        private void CheckControl()
        {
            try
            {
                ...
                CheckControl(txtControlNum);
                CheckControl(dateDoc);
                CheckControl(dateReturn);
        ...
        private void RegisterIncome()
        {
            ...
            try
            {
                ...
                parameters.Add("CREATEDDATE", string.Format("N'{0}'", Tool.DateTimeNow()));
            ...
            catch (MofException ex)
            {
                ...
                else if (ex.errorCode.Equals(2601))
                {
                    txtControlNum.Text = SqlConnector.GetFunction(dbName, "GetControlNum").ToString();
                    RegisterIncome();
                }
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Карттай бичиг хадгалахад алдаа гарлаа: " + ex.Message);
                    throw ex;
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("Карттай бичиг хадгалахад алдаа гарлаа: " + ex.Message);
                throw new MofException("Карттай бичиг хадгалахад алдаа гарлаа!", ex);
            }
        ...
        private void PrintBack()
        {
            try
            {
                CheckControl();
                if (isActionProgress) return;
                Tool.PrintReport(14, "Карттай бичгийн ар талын хэсэг", null);
            }
            catch (MofException ex)
            {
                System.Diagnostics.Debug.WriteLine("Тайланг гаргахад алдаа гарлаа: " + ex.InnerException.Message);
        ...
        private void DeleteIncome()
        {
            ...
            try
            {
                dialogResult = MessageBox.Show(this.Parent, "Мэдээллийг устгахдаа итгэлтэй байна уу?", "Карттай бичиг устгах", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            ...
            {
                System.Diagnostics.Debug.WriteLine("Карттай бичиг устгахад алдаа гарлаа: " + ex.InnerException.Message);
                throw ex;
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("Карттай бичиг устгахад алдаа гарлаа: " + ex.Message);
                throw new MofException("Карттай бичиг устгахад алдаа гарлаа!", ex);
            }
        ...
        private void CheckControl(object control)
        {
            ...
            try
            {
                if (control.ToString().Equals("DevExpress.XtraEditors.DateEdit"))
                {
                    ...
                    {
                        dateEdit.ErrorText = string.Empty;
                        return;
                    }
        ...
        private void SendMail()
        {
            ...
            try
            {
                ...
                if (isEditMode)
                {
                    if (!ckIsMovement.Checked) return;
                    domainId = MainPage.allUser.Select(string.Format("ST_ID = {0}", lkUpMoveStaff.EditValue))[0].ItemArray[4].ToString();
                    branchId = MainPage.allUser.Select(string.Format("ST_ID = {0}", lkUpMoveStaff.EditValue))[0].ItemArray[1].ToString();
                    ...
                    mailParameter = new MailParameter(domainId, txtRegisterNum.Text, txtControlNum.Text, Tool.ConvertNonTimeDateTime(dateDoc.EditValue),
                        txtDocNum.Text, string.Format("{0} - {1}", lkUpLocation.Text, lkUpOrganization.Text), txtFromWho.Text,
                        scannedFileName, Tool.ConvertNonTimeDateTime(dateReturn.EditValue), memoDesc.Text, userName, lkUpMoveStaff.Text);

                    Console.WriteLine(string.Format("1a.Mail is about sending to {0}", domainId));
                    Console.WriteLine(string.Format("2a.Mail is about sending to {0}", headId));
                    Tool.SendMail(mailParameter, headId);
        ...
        private void lkUp_Validating(object sender, CancelEventArgs e)
        {
            ...
            try
            {
                ...
                else if (lkUpEdit.Tag.Equals(2))
                {
                    if (lkUpMoveStaff.Properties.DataSource == null) return;
                    lkUpEdit.ErrorText = "Шилжүүлэх ажилтанг тодорхойлно уу.";
                }
