...
{
    public partial class DomesticList : UserControl
    {
        ...
        public DomesticList(string xmlStr, string dbName, int year)
        {
            InitializeComponent();
            this.xmlStr = xmlStr;
            this.year = year;
        ...
        {
            FormCaption = FormCaption = string.Format("{0}({1} - {2})", MofDoc.Properties.Resources.DomesticListCaption, Tool.ConvertNonTimeDateTime(DateTime.Now.AddDays(-3), "yyyy/MM/dd"), Tool.ConvertNonTimeDateTime(DateTime.Now, "yyyy/MM/dd"));
            ...
            nvItemAddDomestic.Tag = 2;
            nvItemAddDomestic.LinkClicked += new DevExpress.XtraNavBar.NavBarLinkEventHandler(nvItem_LinkClicked);

            nvItemDomesticDelete.Tag = 3;
            nvItemDomesticDelete.LinkClicked += new DevExpress.XtraNavBar.NavBarLinkEventHandler(nvItem_LinkClicked);

            nvItemDomesticEdit.Tag = 4;
            nvItemDomesticEdit.LinkClicked += new DevExpress.XtraNavBar.NavBarLinkEventHandler(nvItem_LinkClicked);
            ...
            this.Disposed += new EventHandler(DomesticList_Disposed);
        }
        ...
        private void InitTreeList()
        {
            ...
            try
            {
                ...
                newColumn = treeList.Columns.Add();
                newColumn.Caption = "Илгээсэн ажилтан";
        ...
        internal void SearchDomestic()
        {
            try
            {
                if (treeList.DataSource == null)
                    return;
                if (search == null) { search = new Search(this.ParentForm, Enum.DocumentType.Domestic, dbName); }
                if (search.IsDisposed) { search = new Search(this.ParentForm, Enum.DocumentType.Domestic, dbName); }
        ...
        internal void AddDomestic()
        {
            DomesticRegister domesticRegister = null;
            try
            {
                domesticRegister = new DomesticRegister(dbName);
                if (domesticRegister.IsDisposed) return;
                domesticRegister.StartPosition = FormStartPosition.CenterParent;
                dialogResult = domesticRegister.ShowDialog();
            ...
            finally { domesticRegister = null; dialogResult = DialogResult.Cancel; }
        }

        internal void DeleteDomestic()
        {
        ...
        private void EditDomestic()
        {
            ...
            try
            {
                ...
                domesticRegister = new DomesticRegister(dbName, listDocument.SingleOrDefault(t => t.PkId.Equals(decimal.Parse(treeList.FocusedNode["PkId"].ToString()))));
                if (domesticRegister.IsDisposed) return;
                domesticRegister.StartPosition = FormStartPosition.CenterParent;
                dialogResult = domesticRegister.ShowDialog();
                if (dialogResult.Equals(DialogResult.Cancel))
            ...
            finally { domesticRegister = null; dataTab = null; }
        }
        ...
        private void PrintList()
        {
            try
            {
                if (treeList.DataSource == null) return;
        ...
        internal void SearchDomestic(Dictionary<string, object> searchData)
        {
            ...
            try
            {
                ...
                if (searchData.ContainsKey("LocationPkId"))
                    cloneDocument = cloneDocument.Where(t => t.LocationPkId.Equals(decimal.Parse(searchData.Single(y => y.Key.Equals("LocationPkId")).Value.ToString()))).ToList();
        ...
        private void DomesticList_Disposed(object sender, EventArgs e)
        {
